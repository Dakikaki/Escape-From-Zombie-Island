...
public class PlayerController : UnitController
{
    ...
    private void EnterTargetingMode()
    {
        ...
        {
            highlightedTiles = currentWeapon.attackPattern.GetValidTargets(gridManager, currentTile, currentWeapon.range);
        }
    ...
    private void HandleTrapPlacementClick()
    {
        ...
        // Check if the player clicked on the highlighted tile (which is their own tile)
        if (targetTile != null && highlightedTiles.Contains(targetTile))
        {
            // --- NEW DATA-DRIVEN CHECK ---
            // Ask the GridManager if a trap is already here.
            ...
            // If the tile is clear, proceed with placement
        ...
        {
            // Clicked an invalid tile, so cancel
            ExitTrapPlacementMode();
        }
    }
